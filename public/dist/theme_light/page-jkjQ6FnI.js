...
import { c as f, a as A } from "./main-DgHGudCI.js";
import { h as C } from "./error-EuuGdfel.js";
import { S as R, P as B, E as I } from "./index-Cc4Suu5E.js";
import { u as M } from "./MarketplaceProvider-CSmUegf3.js";
import { i as k, C as P } from "./vendor-ui-utils-BGnsFFgj.js";
import { a as O } from "./index-CM3cPNp5.js";
...
function G({
  ...
  const { checkNewProducts: u, hasNewProducts: o } = M(), c = T(), h = E, [a, m] = i.useState(!1), [s, w] = i.useState(!1), n = i.useRef(null), [g, N] = i.useState(!1), j = i.useRef(0), v = () => {
    const t = n.current;
    if (t) {
...
function W({
  ...
  const c = Math.ceil(p / u), h = () => {
    ...
    if (c <= 5)
      ...
      for (let s = 2; s <= 4; s++)
        a.push(s);
      a.push("ellipsis"), a.push(c);
    } else if (r >= c - 2) {
      a.push("ellipsis");
      for (let s = c - 3; s <= c; s++)
        a.push(s);
    } else
      a.push("ellipsis"), a.push(r - 1), a.push(r), a.push(r + 1), a.push("ellipsis"), a.push(c);
    return a.map((s, w) => {
      if (s === "ellipsis")
  ...
  return /* @__PURE__ */ e.jsx("div", { className: "h-8 justify-start items-center gap-4 inline-flex", children: /* @__PURE__ */ e.jsxs("div", { className: "justify-start items-center gap-2 flex", children: [
    ...
    /* @__PURE__ */ e.jsx(
      "div",
      {
        onClick: () => r < c && (o == null ? void 0 : o(r + 1)),
        className: f(
          "w-8 h-8 bg-white rounded-2xl border border-[#d9d9d9] flex items-center justify-center",
          r < c ? "cursor-pointer hover:border-[#52c41a]" : "cursor-not-allowed opacity-50"
        ),
...
function ee() {
  const r = z();
  F();
  const [p, u] = i.useState(!1), [o, c] = i.useState([]), [h, a] = i.useState(E[0]), {
    pagination: m,
    handlePageChange: s,
    updateTotal: w,
    updatePageSize: n
  } = Y(), g = i.useCallback(async (l) => {
    try {
      u(!0);
      const d = await O.post("/marketplace/product/search-products", l);
      c(d.data.products), w(d.data.total);
    } catch (d) {
      const S = C(d);
      _.error(S.message);
    } finally {
  ...
  const N = async (l) => {
    ...
    l === "Free" ? d.freeOnly = !0 : l === "Most popular" ? d.sortBy = "popular" : l === "Latest" ? d.sortBy = "latest" : l === "Added" ? d.installedOnly = !0 : d.category = l, g(d);
  }, j = (l) => {
    r(`/marketplace/search-result?search=${l}`);
  }, v = (l) => {
    s(l), g({
      page: l,
      pageSize: y

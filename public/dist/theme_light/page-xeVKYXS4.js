    ...
    function re(t) {
      ...
      this.parse = function(s, h, d) {
        ...
        if (t.newline || (t.newline = function(C, A) {
          ...
          var K = new RegExp(D(A) + "([^]*?)" + D(A), "gm"), ie = (C = C.replace(K, "")).split("\r"), ue = C.split(`
...
` : "\r";
          ...
          var g = function(C, A, K, ie, ue) {
            var he, le, G, J;
            ue = ue || [",", "	", "|", ";", c.RECORD_SEP, c.UNIT_SEP];
            for (var ve = 0; ve < ue.length; ve++) {
              var $ = ue[ve], we = 0, fe = 0, ye = 0;
              G = void 0;
              for (var me = new ee({ comments: ie, delimiter: $, newline: A, preview: 10 }).parse(C), ge = 0; ge < me.data.length; ge++) if (K && se(me.data[ge])) ye++;
              else {
      ...
      this.parse = function(E, oe, ne) {
        ...
        if (t.header && !oe) {
          ...
          for (var C in d) {
            ...
            var K = A, ie = g[A] || 0;
            for (0 < ie && (N = !0, K = A + "_" + ie), g[A] = ie + 1; p.includes(K); ) K = K + "_" + ie;
            p.push(K);
          }
          if (N) {
            var ue = E.split(x);
  ...
  const T = S.useCallback(async () => {
    ...
    try {
      const s = R.getAllTokens().map((g) => g.tokenType), h = {}, d = (g) => I.filter((N, C) => {
        var K;
        return N.token === g && N.amount && (Te(N.to) || ((K = t[C]) == null ? void 0 : K.fullAddress));
      }).map((N, C) => {
        var A;
        return {
    ...
    if (C != null && C.relayerTransactionId && (A != null && A.gasless) && !await xt(C.relayerTransactionId))
